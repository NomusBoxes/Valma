...
class GCodeVisualizer(tk.Tk):
    def __init__(self):
        ...
        self.force_display_label = ttk.Label(self, text="Подача сейчас: ")
        self.force_display_label.pack()
    ...
    def calculate_feedrate(self, prev_point, current_point, max_distance, max_feedrate):
        ...
        if distance == 0:
            return max_feedrate
        return min(max_feedrate, distance * 0.03)  # Предотвращаем деление на ноль
        ...
        for path in self.paths:
            if path:
                ...
                for segment in path:
                    ...
                    self.gcode.append(f"G01 X{x:.2f} Y{y:.2f} F{1000}")
        ...
        try:
            ...
            with serial.Serial(port, baud_rate, timeout=2) as ser:
                ...
                for line in gcode:
                    ...
                    if "F" in line:
                        self.force_display_label.config(text=f"Подача сейчас: { float(line[str(line).find('F')+1:]):.2f}")
